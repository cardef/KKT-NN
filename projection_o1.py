...
def main():
    ...
    parameters = [
        ...
        Variable('P_plus', 0.1, lambda params: 0.9 * params[..., 2]),  # Upper bound depends on P_max (second parameter)
    ...
    validation_loader = model.load_validation_dataset(filepath=validation_filepath, batch_size=512)
    
    # Train the Model
    num_steps = 10000  # Total number of training steps
    batch_size = 512    # Batch size for training
    print("Starting training...")
    model.train_model(num_steps=num_steps, batch_size=batch_size, validation_loader=validation_loader)
