...
Assume that \(g_i\) satisfies Slater's condition.
\(x^* \in \mathcal{D}\) is optimal if and only if there are \(\lambda\)
and \(\nu\) that, with \(x^*\), satisy the Karush-Kuhn-Tucker conditions
(KKT):

\[
\]

\section{Case of study}\label{case-of-study}
