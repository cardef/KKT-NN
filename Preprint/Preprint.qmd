...
The objective is to process a batch of $B$ problem parameters $\Theta \in \mathbb{R}^{B \times k}$, $\Theta_i = \theta^{(i)}$ and have as output $\hat{X}$, $\hat\Lambda$, $\hat{N}$. Assuming to know the range of feasibility of each parameter, $\Theta$ will be scaled in order to obtain $\Theta^\prime \in [-1 , 1]^{B \times k}$ which will be used to fed the network, aiding the convergence. network architecture is shown in @fig-architecture. An embedding for $\Theta^\prime$ is learned through a MLP, where each layer has a skip connection to the previous one. This embedding is then used as input to three MLPs, predicting respectively $\hat{X}$, $\hat\Lambda$, $\hat{N}$. A softplus function is applied to the network predicting $\hat\Lambda$ to ensure its feasibility.
...
## Projection onto a convex polythope
...
log_df <- read_csv("../Projection/log.csv", show_col_types=FALSE)
...
ggpairs(params_df, mapping=aes(fill=sum),columns = 2:8, upper = list(continuous = "blank"), diag = list(continuous = "blankDiag"), lower = list(continuous = ggally_mysmooth), legend=8) + theme(legend.position="bottom")
```
#### Evaluation
...
#|      - MAPE
#|      - $R^{2}$
metrics_df <- read_csv("../Projection/metrics.csv", show_col_types=FALSE)

extrafont::loadfonts(quiet = TRUE)
ggplot(metrics_df %>%select(Step, MAE), aes(x=Step, y=MAE,)) + geom_line()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
ggplot(metrics_df %>%select(Step, RMSE), aes(x=Step, y=RMSE,)) + geom_line()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
ggplot(metrics_df%>%select(Step, MAPE), aes(x=Step, y=MAPE,)) + geom_line()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
ggplot(metrics_df%>%select(Step, R2), aes(x=Step, y=R2,)) + geom_line()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
```
```{r}
