...
extrafont::loadfonts(quiet = TRUE)
ggplot(metrics_df %>%select(Sample, optimality_gap), aes(x=optimality_gap,)) + geom_density()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
ggplot(metrics_df %>%select(Sample, inequality_violation), aes(x=inequality_violation,)) + geom_density()   + scale_colour_tableau(
  palette = "Tableau 10", direction=-1
) + scale_y_log10("Value") + theme_light()
```
```{r}
