...
class KKT_NN:
    def __init__(self):
        ...
        self.n = 1000
        t = torch.linspace(0, 1, self.n, dtype=torch.float64)
        self.true = (
            0.8 * ((torch.sin(50 * t) + torch.sin(10 * t)) / 2) + 0.1 * t
        ).to(dtype=torch.float64)  # Profilo di elevazione reale
        self.noisy = self.true + 0.1 * torch.rand(self.n, dtype=torch.float64)  # Dati misurati con rumore

        self.device = torch.device("cpu")
        self.net = Net(self.noisy, self.n).to(self.device)
        self.batch_size = 64
        self.n_iter = 0
        self.tolerance = 1e-6
        ...
        self.D = torch.tensor(self.D, dtype=torch.float64, device=self.device)

        self.D2 = np.eye(self.n) - 2 * np.eye(self.n, k=1) + np.eye(self.n, k=2)
        self.D2 = self.D2[:-2, :]
        self.D2 = torch.tensor(self.D2, dtype=torch.float64, device=self.device)
        ...
        self.complementarity_weight_concavity = 1.0  # Incrementato per maggiore importanza
        self.complementarity_weight_variation = 1.0  # Incrementato per maggiore importanza
    ...
    def kkt_loss(self, x, rho, kappa, lambda_, mu):
        ...
        grad_f = torch.autograd.grad(cost_mean, x, create_graph=True)[0]          # Shape: (batch_size, n)
        grad_g2 = torch.autograd.grad(concavity_mean, x, create_graph=True)[0]    # Shape: (batch_size, n)
        grad_g1 = torch.autograd.grad(variation_mean, x, create_graph=True)[0]     # Shape: (batch_size, n)
        ...
        lambda_active = torch.where(active_variation, lambda_, torch.zeros_like(lambda_))  # Shape: (batch_size,)
        mu_active = torch.where(active_concavity, mu, torch.zeros_like(mu))               # Shape: (batch_size,)
        ...
        grad_L = grad_f + lambda_active * grad_g1 + mu_active * grad_g2  # Shape: (batch_size, n)
        ...
        complementarity_concavity = (mu_active.squeeze(1) * torch.relu(-concavity)).pow(2).mean()  # Scalar
        complementarity_variation = (lambda_active.squeeze(1) * torch.relu(-variation)).pow(2).mean()  # Scalar
    ...
    def training_step(self):
        ...
        rho_kappa = self.soboleng.draw(self.batch_size, dtype=torch.float64)
        rho = rho_kappa[..., 0]  # Valori tra 0.75 e 1.25
        kappa = rho_kappa[..., 1]  # Valori tra 0.75 e 1.25
        
        # Forward pass
        x, lambda_, mu = self.net(torch.stack([rho, kappa], 1))
        ...
        self.tb_logger.add_scalar("Loss/Cost", self.objective_fun(x).mean().item(), self.n_iter)
        self.tb_logger.add_scalar(
        ...
        self.tb_logger.add_scalar("Loss/ConcComp", concavity_complementary.item(), self.n_iter)
        self.tb_logger.add_scalar("Loss/VarComp", variation_complementary.item(), self.n_iter)
        
        # Logging dei moltiplicatori di Lagrange
        self.tb_logger.add_histogram("Lambda", lambda_.detach().cpu().numpy(), self.n_iter)
        ...
        self.tb_logger.add_scalar("Concavity_Violation_Percentage", concavity_violation, self.n_iter)
        self.tb_logger.add_scalar("Variation_Violation_Percentage", variation_violation, self.n_iter)
        
        self.tb_logger.flush()
        self.n_iter += 1
        return total_loss.item()

    def validation_step(self, rho, kappa, y, mu, lambda_):
        self.net.eval()
        with torch.no_grad():
            x, pred_lambda, pred_mu = self.net(torch.stack([rho, kappa], 1))
        ...
        self.tb_logger.add_scalar("Val/ConcComp", concavity_complementary.item(), self.n_iter)
        self.tb_logger.add_scalar("Val/VarComp", variation_complementary.item(), self.n_iter)
        self.tb_logger.add_scalar("Val/Lambda_Max", pred_lambda.max().item(), self.n_iter)
...
if __name__ == "__main__":
    ...
    for epoch in range(10000):
        ...
        for rho, kappa, y, mu, lambda_ in loader:
            model.validation_step(rho, kappa, y, mu, lambda_)
        pbar.update(1)
