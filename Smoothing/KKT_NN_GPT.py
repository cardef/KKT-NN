...
class ConvNet(nn.Module):
    def __init__(self, signal, num_residual_blocks=4, hidden_channels=64, device=torch.device("cpu")):
        super(ConvNet, self).__init__()
        self.signal = signal

        # Rete completamente connessa per elaborare i parametri globali
        self.fc = nn.Sequential(
            nn.Linear(2, 128),
            nn.LeakyReLU(),
            nn.Linear(128, 256),
            nn.LeakyReLU(),
            nn.Linear(256, hidden_channels-1),
        ).to(device)

        # Blocchi residui convoluzionali
        self.residual_blocks = nn.Sequential(
            *[
                ConvResidualBlock(hidden_channels, hidden_channels)
                for _ in range(num_residual_blocks)
            ]
        ).to(device)

        # Strato finale per generare il segnale smussato
        self.conv_out = nn.Conv1d(hidden_channels, 1, kernel_size=1).to(device)

        # Strati per lambda e mu
    ...
    def forward(self, X):
        rho_kappa = X[:, :2]  # I primi due valori (rho e kappa)
        noisy_input = self.signal.unsqueeze(
            0
        )  # Rendi noisy input a forma [1, N] per concatenazione
        features = self.fc(rho_kappa)  # [batch_size, hidden_channels]
        features = features.unsqueeze(-1)  # [batch_size, hidden_channels, 1]

        # Replicare le features per la lunghezza della sequenza
        features = features.repeat(
            1, 1, self.signal.size(0)
        )  # [batch_size, hidden_channels, N]

        # Concatenare noisy_input con features lungo la dimensione dei canali
        combined_input = torch.cat(
            [noisy_input.unsqueeze(1).repeat(features.size(0), 1, 1), features], dim=1
        )  # [batch_size, hidden_channels+1, N]
        start = time.time()
        out = self.residual_blocks(combined_input)  # [batch_size, hidden_channels, N]
        x = self.conv_out(out).squeeze(1)  # [batch_size, N]
        end = time.time()
        print(end-start)
        # Moltiplicatori di Lagrange
        lambda_ = self.fc_lambda(features.mean(dim=2))  # [batch_size, 1]
        mu = self.fc_mu(features.mean(dim=2))  # [batch_size, 1]

        return self.signal + x, lambda_.squeeze(), mu.squeeze()
...
class KKT_NN:
    def __init__(self):
        ...
        self.net = ConvNet(self.noisy, num_residual_blocks=2, hidden_channels=128, device=self.device).to(self.device)
