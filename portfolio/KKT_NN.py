...
class KKT_NN():
    ...
    def kkt_loss(self, S, mu, x, sol, lambd, nu):
        ...
        grad_f = torch.bmm(S.view(512, 20, 20), sol.unsqueeze(-1)).squeeze()
        ...
        loss_stationarity = torch.mean(torch.norm(stationarity, dim=1))
        loss_g_eq = torch.mean(torch.norm(g_eq, dim=0))
        loss_g_ineq = torch.mean((torch.norm(torch.relu(g_ineq))))
        loss_complementary = torch.mean(torch.abs(complementary))
    ...
    def training_step(self, S, mu, X, y):
        ...
        mu = 2.0 * torch.rand(512, 20).to(dtype=torch.float32, device=self.device) - 1.0
        X = (torch.min(mu, 1)[0] + 0.1)+ torch.rand(512).to(dtype=torch.float32, device=self.device) * ((torch.max(mu,1)[0] - 0.1) - torch.min(mu, 1)[0] + 0.1)
        X = X.unsqueeze(-1)
        def closure():
