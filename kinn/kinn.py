...
class KINN:
	...
	def __init__(
		...
		self.metrics = {param.name: [] for param in self.problem.parameters}
		self.metrics.update({f"{decision_variable.name}_optimal": [] for decision_variable in self.problem.decision_variables})
		self.metrics.update({f"{decision_variable.name}_predicted": [] for decision_variable in self.problem.decision_variables})
		self.metrics.update({
			"predicted_cost": [],
			"optimal_cost": [],
			"equality_violation": [],
	...
	def validation_step(self, validation_loader):
		...
		with torch.no_grad():
			for params, solutions in validation_loader:
				...
				if len(equality_violations) > 0: equality_violations = torch.tensor(equality_violations).T.mean(-1).tolist()
				if len(inequality_violations) > 0: inequality_violations = torch.tensor(inequality_violations).T.mean(-1).tolist()

				for param in self.problem.parameters:
						self.metrics[param.name].extend(params_dict[param.name].tolist())

				for i, variable in enumerate(self.problem.decision_variables):
					self.metrics[f"{variable.name}_optimal"].extend(solutions[..., i].squeeze().tolist())
					self.metrics[f"{variable.name}_predicted"].extend(decision_vars[..., i].squeeze().tolist())
				self.metrics["predicted_cost"].extend(predicted_cost.tolist())
				self.metrics["optimal_cost"].extend(optimal_cost.tolist())
				self.metrics["equality_violation"].extend(equality_violations)
				self.metrics["inequality_violation"].extend(inequality_violations)
 ...
 """
				...
				""" print(
					f"Optimality Gap={np.median(optimality_gap):.4f}, Optimality Gap Percentage={np.median(optimality_gap_percentage):.4f}, Equality violation={np.median(equality_violation):.4f}, Inequality violation={np.median(inequality_violation):.4f}"
				) """
