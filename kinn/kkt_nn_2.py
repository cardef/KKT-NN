...
class KKT_NN:
    ...
    def lagrangian(self, decision_var, dual_eq, dual_ineq, params_dict):
            ...
            """
            # Calcola il costo
            lagrangian = self.problem.cost_function(decision_var, params_dict)  # Assicurati che cost_function abbia un metodo compute_pytorch
            #lagrangian = cost.squeeze(0)  # Scalar

            # Aggiungi i termini duali per i vincoli di uguaglianza
            if dual_eq is not None:
                expr = torch.cat([constraint.get_constraints(decision_var, params_dict) for constraint in self.problem.constraints[:len(self.num_eq_constraints)]], -1)
    ...
    def kkt_loss(self, decision_vars, dual_eq_vars, dual_ineq_vars, params_dict):
        ...
        elif sum(self.num_eq_constraints) == 0:
        ...
        elif sum(self.num_ineq_constraints) == 0:
    ...
    # Vettorializza la funzione del gradiente del Lagrangiano
        grad_L = vmap(grad(self.lagrangian, argnums=0), in_dims=in_dims)(decision_vars, dual_eq_vars, dual_ineq_vars, params_dict)
        ...
        for i, constraint in enumerate(self.problem.constraints):
            expr = constraint.get_constraints(decision_vars, params_dict)  # Forma: (batch_size, n_constraints)
            if constraint.type == 'equality':
        ...
        if dual_ineq_vars is not None:
                expr = torch.cat([constraint.get_constraints(decision_vars, params_dict) for constraint in self.problem.constraints[len(self.num_eq_constraints):]], -1)
    ...
    def training_step(self, batch_size):
        ...
        stationarity_loss, feasibility_loss, complementarity_loss = self.kkt_loss(decision_vars, dual_eq_vars, dual_ineq_vars, params_dict)
